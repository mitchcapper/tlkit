 ...
 * MD5TEE
 */
size_t pump(void *ptr,size_t isz,size_t cnt) {
  size_t i = fread(ptr,isz,cnt,stdin);
  if (i) fwrite(ptr,isz,i,stdout);
  return i;
}
...
void sum(FILE *res) {
  ...
  for(i=0;i<16;i++) fprintf(res,"%.2x", digest[i]);
  putc('\n',res);
}
