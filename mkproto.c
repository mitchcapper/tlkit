 ...
 *    mkproto 1
 * SUMMARY
 *    make prototypes for functions
 * SYNOPSIS
 *    *mkproto* _[-n]_ _[-s]_ _[-p]_ _[_ _file_ _]_ _..._
 * DESCRIPTION
 *    *Mkproto* takes as input one or more C source code files, and
 *    produces as output (on the standard output stream) a list of
 *    function prototypes (a la _ANSI_ ) for the external functions 
 *    defined in the given source files. This output, redirected to a
 *    file, is suitable for *#include* 'ing in a C source file.
 *
 *    The function definitions in the original source
 *    may be either _"old-style"_ (in which case appropriate prototypes 
 *    are generated for the functions) or _"new-style"_ (in which the 
 *    definition includes a prototype already).
 * OPTIONS
 *    * *-n* 
 *      causes the line number where each function was defined to be 
 *      prepended to the prototype declaration as a comment.
 *    * *-s*
 *      causes prototypes to be generated for functions declared
 *      *"static"* as well as extern functions.
 *    * *-p*
 *      causes the prototypes emitted to be only readable by _ANSI_
 *      compilers. Normally, the prototypes are _"macro-ized"_ so that
 *      compilers with *__STDC__* not defined don't see them.
 *
 *    If files are specified on the command line, then a comment 
 *    specifying the file of origin is emitted before the prototypes 
 *    constructed from that file. If no files are given, then no 
 *    comments are emitted and the C source code is taken from the 
 ...
 *    being run through *mkproto*. The -n option is unlikely to work as
 *    desired on the output of a preprocessor.
 *
 *    Typedef'd types aren't correctly promoted, e.g. for
 *
 *    :    typedef schar char; int foo(x) schar x;...
 *
 *    *mkproto* incorrectly generates the prototype int foo(schar x) rather 
 *    than the (correct) int foo(int x).
 *
 *    Functions named _"inline"_ with no explicit type qualifiers are not
 *    recognized.
 * SEE ALSO
 *    *cc(1),*
 *    *lint(1)*
 ...
 *    Eric R. Smith.
 * NOTE
 *    There is no warranty for this program (as noted above, it's
 *    guaranteed to break sometimes anyways!). *Mkproto* is in the 
 *    public domain.
 *--
 */
